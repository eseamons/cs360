...
router.get('/user_added_restaurants', function(req, res, next) {

  var user_id = req.query.user_id;

  Restaurant.find({"user_id": user_id},function(err, restaurants){
    if(err){ return next(err); }
    res.json(restaurants);
  });
});

//coll.find({"user_id": "570c6e223681e57138f395e2"});
...
router.put('/restaurants/:restaurant/addConnection', function(req, res, next) {
  req.restaurant.addConnection(function(err, restaurant){
    if (err) { return next(err); }
    res.json(restaurant);
  });
});
